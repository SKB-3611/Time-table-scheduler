...
const renderTimeSlot = (slot: TimeSlot,showtime:boolean=false) => (
  <div className={`p-2 rounded ${ slot?.isReplaced ? 'bg-green-50' : slot.subject === 'Break' ? 'bg-gray-50' : 'bg-blue-50' }`}>
    {slot?.subject !== 'Break' ? (
      <>
      ...
      <div className="text-center font-medium text-gray-500 h-full">Break</div>
    )}
  </div>
);
